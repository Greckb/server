...
// Ruta real '/pages/profile'
router.get('/pages/profile', (req, res) => {
    const { tab = '' } = req.query;

    console.log(tab);

    // Obtener los datos mock correspondientes según la pestaña (tab)
    const responseData = data[tab];

    res.status(200).json(responseData);
});

// Ruta real '/pages/profile-header'
router.get('/pages/profile-header', (req, res) => {
    const responseData = data.profileHeader;

    res.status(200).json(responseData);
});

// Ruta real '/pages/profile-table'
router.get('/pages/profile-table', (req, res) => {
    const { q = '' } = req.query;
